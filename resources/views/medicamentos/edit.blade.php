...
@section('content')
  <div class="card">
    <div class="card-header d-flex flex-wrap justify-content-between align-items-center gap-2">
      <h4 class="fw-semibold mb-0">Editar medicamento</h4>

      <a href="{{ route('medicamentos.show', $medicamento) }}" class="btn btn-outline-secondary">
        ...
        <div class="row g-3">
          ...
          <div class="col-md-6">
            ...
            <input
              ...
              value="{{ old('fabricante', $medicamento->fabricante) }}" />
            @error('fabricante')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
          ...
          <div class="col-md-3">
            ...
            <select id="tipo" name="tipo" class="form-select @error('tipo') is-invalid @enderror" required>
              @foreach (\App\Enums\TipoMedicamento::cases() as $tipo)
                <option value="{{ $tipo->value }}" @selected(old('tipo', $medicamento->tipo?->value) === $tipo->value)>
                  {{ $tipo->label() }}
                </option>
              @endforeach
            </select>
          ...
          <div class="col-md-3 d-none" data-mostrar-se-tipo="miligrama">
            ...
            <input
              ...
              value="{{ old('tamanho_vasilhame', $medicamento->tamanho_vasilhame) }}"
              data-quantidade
              inputmode="decimal"
              placeholder="Ex.: 10,5" />
          ...
          <div class="col-md-3">
            ...
            <select id="grupo_id" name="grupo_id" class="form-select @error('grupo_id') is-invalid @enderror">
              ...
              @foreach ($grupos as $grupo)
                <option value="{{ $grupo->id }}" @selected(old('grupo_id', $medicamento->grupo_id) == $grupo->id)>
                  {{ $grupo->nome }}
                </option>
              @endforeach
            </select>
          ...
          <div class="col-md-3">
            ...
            <select id="status" name="status" class="form-select @error('status') is-invalid @enderror">
              @foreach (\App\Enums\StatusAtivoInativo::cases() as $status)
                <option value="{{ $status->value }}" @selected(old('status', $medicamento->status?->value) === $status->value)>
                  {{ $status->label() }}
                </option>
              @endforeach
            </select>
          ...
          <div class="col-md-3">
            ...
            <input
              ...
              value="{{ old('ultimo_valor_pago', $medicamento->ultimo_valor_pago) }}"
              data-moeda
              inputmode="numeric"
              placeholder="R$ 0,00" />
          ...
          <div class="col-md-3">
            ...
            <input
              ...
              value="{{ old('valor_venda', $medicamento->valor_venda) }}"
              data-moeda
              inputmode="numeric"
              placeholder="R$ 0,00" />
            @error('valor_venda')
          ...
          <div class="col-md-3">
            ...
            <input
              ...
              value="{{ old('estoque_minimo', $medicamento->estoque_minimo) }}" />
          ...
          <div class="col-md-3">
            ...
            <input
              ...
              value="{{ old('estoque_medio', $medicamento->estoque_medio) }}" />
            @error('estoque_medio')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
          ...
          <div class="col-md-3">
            ...
            <input
              ...
              value="{{ old('feegow_aplicacao_id', $medicamento->feegow_aplicacao_id) }}" />
          ...
          <div class="col-md-3">
            ...
            <div class="form-check form-switch mt-2">
              <input
                ...
                name="gera_aplicacao"
                value="1"
                @checked(old('gera_aplicacao', $medicamento->gera_aplicacao)) />
              <label class="form-check-label" for="gera_aplicacao">Sim, gera aplicação</label>
            </div>
          </div>

          <div class="col-12 d-flex justify-content-end gap-2">
            <a href="{{ route('medicamentos.show', $medicamento) }}" class="btn btn-outline-secondary">Cancelar</a>
    ...
    <div class="card-footer d-flex justify-content-between align-items-center flex-wrap gap-2">
      <small class="text-muted">A exclusão é lógica: o medicamento sai da listagem, mas permanece no banco.</small>

      <form method="POST" action="{{ route('medicamentos.destroy', $medicamento) }}" data-confirmar="Deseja realmente excluir este medicamento?">
        @csrf
        @method('DELETE')
        <button type="submit" class="btn btn-outline-danger">
          <i class="ri-delete-bin-7-line me-1"></i>Excluir medicamento
        </button>
      </form>
    </div>
  </div>
